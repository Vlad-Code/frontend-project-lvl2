...
beforeAll(() => {
  ...
  resultForTree = readFileSync(`${__dirname}/../__fixtures__/resultForTreeStylish.txt`, 'utf-8');
});
...
test('check function for tree json files', () => {
  ...
  expect(genDiff(filePathJson1, filePathJson2)).toEqual(resultForTree);
});

test('check function for tree yaml files', () => {
  ...
  expect(genDiff(filePathYaml1, filePathYaml2)).toEqual(resultForTree);
});
